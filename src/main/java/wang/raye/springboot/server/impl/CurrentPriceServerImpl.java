...
//		}
		...
		log.error("线程 "+exchange+ " 实时价格 :"+Thread.currentThread().getName()+"异常终止....." + e.getMessage());
	}
...
//		}
	...
	@Override
	public boolean gatePrice(String exchange) {
		// Create a GateClient
		GateClient gateClient = new GateClient();

		// Perform a getMarkets request on the public api
		Map<String, Market> marketMap = gateClient.getPublicApi().getMarket();

		SymbolsCriteria cond = new SymbolsCriteria();
		SymbolsCriteria.Criteria criteria = cond.createCriteria();
		criteria.andExchangeEqualTo(exchange);
		List<Symbols> symbolsList = symbolsMapper.selectByExample(cond);
		for (String marketName : marketMap.keySet()) {
			boolean isNew = true;
			Symbols updateSymbol = new Symbols();
			for (Symbols symbol:symbolsList) {
				if(symbol.getSymbol().equals(marketName)) {
					isNew = false;
					updateSymbol = symbol;
					break;
				}
			...
			if (isNew) {
				Alert alert = new Alert();
				alert.setExchange(exchange);
				alert.setSymbol(marketName);
				alert.setType(NEW);
				alert.setStatus("0");
				alert.setPrice(marketMap.get(marketName).getLast().doubleValue());
				alert.setPeriod("0m");
				alert.setTime(new Date());
				alert.setCount(1);
				alert.setSend("1");
				...
				Symbols record = new Symbols();
				record.setExchange(exchange);
				record.setSymbol(marketName);
				record.setPrice(marketMap.get(marketName).getLast().doubleValue());
				record.setTime(new Date());
				symbolsMapper.insert(record);

				this.sendNew(exchange,marketName,String.valueOf(marketMap.get(marketName).getLast()));
			} else {
				updateSymbol.setPrice(marketMap.get(marketName).getLast().doubleValue());
				updateSymbol.setTime(new Date());
				symbolsMapper.updateByPrimaryKey(updateSymbol);
			}
		}
