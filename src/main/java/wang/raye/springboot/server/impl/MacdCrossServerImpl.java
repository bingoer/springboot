...
import de.elbatya.cryptocoins.bittrexclient.GateClient;
...
import de.elbatya.cryptocoins.bittrexclient.api.model.publicapi.gate.GateInterval;
import de.elbatya.cryptocoins.bittrexclient.api.model.publicapi.gate.GateIntervalMap;
...
public class MacdCrossServerImpl implements MacdCrossServer{
	...
	@Override
	public boolean gateMacdCross(String exchange, GateInterval periods) {
		// Create a GateClient
		...
		for (Symbols symbols:symbolsList) {
			...
			String period = periods.getIntervalId();
			//调api获取k线
			List<de.elbatya.cryptocoins.bittrexclient.api.model.publicapi.gate.Candle> candleList = null;
			int groupSec = GateIntervalMap.ENUMMAP.get(periods);
			int rangeHour = GateIntervalMap.ENUMMAP.get(GateInterval.THREE_DAILY);
			try {
				candleList = (List)gateClient.getPublicApi().getKline(symbols.getSymbol(), groupSec, rangeHour).unwrap();
			...
			List<Candlestick> candlestickList = ParseUtils.parseGateCandlestick(candleList);
...
//			String statusDoji = quotaCrossUtils.getDojiCross(dojiBeanList, candlestickList);
			...
			// 保存DOJI数据
			this.saveCross(exchange, symbol, period, DOJI, statusDoji, secondCandle, secondDojiBean.getDoji(), secondDojiBean.getHhv(), secondDojiBean.getDownCount(), maxPrice, minPrice);
		}
		catch (Exception e) {
	...
	private void saveCross (String exchange, String symbol, String period, String type, String status,
		...
		if (!("1".equals(status) || "3".equals(status))) {
			List<MacdCrossHistory> macdCrossHistoryExistList = this.getMacdCrossHistory(exchange, symbol, period, type, status);
			if (macdCrossHistoryExistList.size() > 0) {
				if(!((macdCrossHistoryExistList.get(0).getStatus().equals("2") && status.equals("2"))
						||(macdCrossHistoryExistList.get(0).getStatus().equals("4") && status.equals("4")))) {
					this.modMacdCrossHistory(macdCrossHistoryExistList,Double.valueOf(lastCandle.getClose()), quota1, quota2, quota3);
				}
			} else {
				this.addMacdCrossHistory(exchange, symbol, period, type, status, Double.valueOf(lastCandle.getClose()), quota1, quota2, quota3);
	...
	private void saveAlert (String exchange, String symbol, String period, String type, String status,
		...
		if (null != alertSettingExistList && alertSettingExistList.size() > 0) {
			...
			// 交易基础币
//			String base = alertSettingExistList.get(0).getBase();
			String base = "BTC";
...
//		criteriaAlert.andExchangeEqualTo(exchange);
	...
	private void sendCross(String sckey,String exchange, String symbol, String period, String type, String status, Candlestick lastCandle, List<PositionBean> fibonacciList){
		...
		sendBean.setPrice(Double.valueOf(lastCandle.getClose()));
