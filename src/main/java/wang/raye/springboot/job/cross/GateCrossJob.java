...
public class GateCrossJob implements Job {
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		logger.info("线程 "+ GATE +" 2h指标数据保存:"+Thread.currentThread().getName()+"运行开始.....");
		crossServer.saveCross(GATE, CommonInterval.TWO_HOURLY);
		logger.info("线程 "+ GATE +" 2h指标数据保存:"+Thread.currentThread().getName()+"运行结束.....");
	}
